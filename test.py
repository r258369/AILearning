...
def generate_structured_content_with_gemini(topic, level):
    prompt = f'''
Topic: {topic}\nLevel: {level}
...
def generate_pdf_from_content(content_dict, topic, user_id):
    # Use reportlab to generate a PDF in memory and return as base64
    ...
    try:
        ...
        c.drawString(40, y, "📝 Study Notes")
        y -= 20
        c.setFont("Helvetica", 11)
        for line in content_dict["notes"].split('\n'):
            line = line.strip()
            if line:
                textobject = c.beginText()
                textobject.setTextOrigin(40, y)
                textobject.textLine(line)
                c.drawText(textobject)
                y -= 15
            if y < 60:
                c.showPage(); y = height - 40; c.setFont("Helvetica", 11)
        y -= 10

        # Assignment Questions
        c.setFont("Helvetica-Bold", 14)
        c.drawString(40, y, "📄 Assignment Questions")
        y -= 20
        c.setFont("Helvetica", 11)
        for line in content_dict["assignment"].split('\n'):
            line = line.strip()
            if line:
                textobject = c.beginText()
                textobject.setTextOrigin(40, y)
...
def save_note_to_firestore(user_id, topic, pdf_content_base64):
    if pdf_content_base64:
        user_profile_ref = db.collection('user_profiles').document(user_id)
        user_profile_ref.set({
            'notes_pdfs': firestore.ArrayUnion([{ 'topic': topic, 'pdf_url': pdf_content_base64 }]),
            'last_updated': firestore.SERVER_TIMESTAMP
        }, merge=True)
        print(f"PDF content saved to Firestore for topic: {topic}")
    else:
...
Important: ONLY return the HTML **inside the body tag**, NOT the full HTML structure 
...
def quiz_view(request):
    ...
    if request.method == 'POST':
        ...
        if 'quiz_preferences_submitted' in request.POST:
            ...
            Important: ONLY return the HTML **inside the body tag**, NOT the full HTML structure (no `<!DOCTYPE>`, `<html>`, `<head>`, or `<body>` tags).
            """
...
def notes_view(request):
    ...
    if firebase_uid:
        ...
        if doc.exists:
            user_profile = doc.to_dict()
            notes_pdfs = user_profile.get('notes_pdfs', [])
            
            # Get course title from specific_goals
            specific_goals = user_profile.get('specific_goals', '')
            if specific_goals and specific_goals.strip():
                goals_list = [goal.strip() for goal in specific_goals.split(',')]
                course_title = goals_list[0].title()
            else:
                course_title = "My Learning Course" # Fallback if specific_goals is empty

    context = {
        'user_profile': user_profile,
        'notes_pdfs': notes_pdfs,
        'course_title': course_title, # Pass the course title to the template
    }
...
def generate_study_notes(request):
    ...
    try:
        ...
        # preferred_subjects = user_profile_data.get('preferred_subjects', 'general knowledge') # No longer needed directly here
        ...
        for topic in raw_topics:
            ...
            gemini_content = generate_structured_content_with_gemini(topic, skill_level)
            ...
            if content_sections.get('notes') and content_sections.get('assignment'):
                ...
                if pdf_content_base64:
                    # Step 4: Save PDF content to Firestore
                    save_note_to_firestore(firebase_uid, topic, pdf_content_base64)
                    new_notes_generated.append({'topic': topic, 'pdf_url': pdf_content_base64})
                else:
